...
| `docs/` | Architecture spec, runbook, voice setup notes |
...
Watch events: `docker compose exec mosquitto mosquitto_sub -t 'pihome/#' -v`
...
uv run ruff check .
uv run pihome validate-jobs --jobs-dir ../jobs
```

Run the service outside Docker (uses `PIHOME_*` env or `.env` defaults):
...
```sh
uv run pihome execute-job < request.json
```
