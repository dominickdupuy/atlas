...
by ATLAS_PROFILE: `dev` is fully stubbed (zero credentials), `prod` wires
...
from atlas.connectors.infrastructure.stubs import (
    LogNotifier,
    StubLlmProvider,
    StubMcpClient,
    StubWeather,
...
def build_connectors(settings: Settings) -> Connectors:
    ...
    from atlas.connectors.infrastructure.mcp_client import McpHttpClient
    from atlas.connectors.infrastructure.weather_http import OpenMeteoWeatherClient

    if not settings.anthropic_api_key:
        raise RuntimeError("ATLAS_PROFILE=prod requires ANTHROPIC_API_KEY")
...
def build_notifier(settings: Settings) -> Notifier:
    if settings.ntfy_token:
        from atlas.connectors.infrastructure.ntfy_notifier import NtfyNotifier

        return NtfyNotifier(settings.ntfy_url, settings.ntfy_topic, settings.ntfy_token)
    logger.info("no ATLAS_NTFY_TOKEN configured; notifications go to the log")
    return LogNotifier()
