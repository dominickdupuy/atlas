"""Dev-profile stand-ins (ATLAS_PROFILE=dev): the whole stack runs on a
...
_CANNED: dict[str, JsonValue] = {
    ...
    "github.list_notifications": [
        {"repo": "atlas", "subject": "CI passed on main", "reason": "ci_activity"},
    ],
...
class StubMcpClient:
    ...
    async def call_tool(self, call: ToolCall) -> ToolResult:
        ...
        delay = float(os.environ.get("ATLAS_STUB_TOOL_DELAY", "0"))
        if delay > 0:
            await asyncio.sleep(delay)
        block = float(os.environ.get("ATLAS_STUB_TOOL_BLOCK", "0"))
        if block > 0:
