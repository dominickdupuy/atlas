...
from atlas.presentation.http.runs import build_runs_timeline
...
class PanelRenderer:
    ...
    def _timeline_context(self, runs: list[JobRun]) -> dict[str, object]:
        timeline = build_runs_timeline(
            ...
            runs,
            self._app.clock.now(),
            stub_jobs=self._app.settings.profile != "prod",
        )
        return {
            "pending": timeline.pending,
            "pending_more": timeline.pending_more,
            "history": timeline.history,
            "history_more": timeline.history_more,
        }
    ...
    def render_board(self) -> str:
        ...
        """
        return self._env.get_template("board.html").render(asset_version=self._asset_version)
