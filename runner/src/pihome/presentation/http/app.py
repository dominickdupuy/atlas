...
from pihome.presentation.http.auth import BearerAuthMiddleware
from pihome.presentation.http.panels import PanelRenderer
from pihome.presentation.http.routers import approvals, dashboard, events, health, jobs
...
def create_app(application: Application) -> FastAPI:
    ...
    async def lifespan(_: FastAPI) -> AsyncIterator[None]:
        ...
        logger.info("pi-home runner up (profile=%s)", application.settings.profile)
        try:
            yield
        finally:
            for task in tasks:
    ...
    app = FastAPI(title="pi-home", lifespan=lifespan, docs_url=None, redoc_url=None)
