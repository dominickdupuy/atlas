...
ATLAS_STUB_TOOL_DELAY (async — the child's own timeout can fire) and
ATLAS_STUB_TOOL_BLOCK (blocking — only the parent's kill ends it).
"""
...
from atlas.jobs.domain.run import RunRequest
from atlas.jobs.infrastructure import subprocess_launcher
from atlas.jobs.infrastructure.subprocess_launcher import SubprocessJobLauncher
from atlas.shared.ids import RunId
...
def _dev_profile(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ATLAS_PROFILE", "dev")
    monkeypatch.delenv("ATLAS_STUB_TOOL_DELAY", raising=False)
    monkeypatch.delenv("ATLAS_STUB_TOOL_BLOCK", raising=False)
...
async def test_child_self_timeout_reports_timed_out(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ATLAS_STUB_TOOL_DELAY", "10")
...
async def test_parent_kills_a_wedged_child(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ATLAS_STUB_TOOL_BLOCK", "30")
...
async def test_launcher_surfaces_crash_as_typed_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ATLAS_PROFILE", "prod")
